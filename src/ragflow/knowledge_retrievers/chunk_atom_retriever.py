...
from src.ragflow.knowledge_retrievers.mixins.chroma_mixin import ChromaMixin, load_vector_store
...
class ChunkAtomRetriever(BaseRetriever, ChromaMixin):
    ...
    def _atom_info_tuple_to_class(self, atom_retrieval_info: List[Tuple[str, Document, float]]) -> List[AtomRetrievalInfo]:
        ...
        chunk_results = []
        ids, chunks = load_callable(
            module_path=self.vector_store_config["id_document_loading"]["module_path"],
            name=self.vector_store_config["id_document_loading"]["func_name"],
        )(**self.vector_store_config["id_document_loading"].get("args", {}))
        for chunk in chunks:
            if chunk.metadata["id"] in source_chunk_ids:
                chunk_results.append(chunk)
        chunk_id_to_content = {
            chunk.metadata["id"]: chunk.page_content
    ...
    def retrieve_atom_info_through_atom(
        self, queries: Union[List[str], str], **kwargs,
    ...
    def retrieve_contents_by_query(self, query: str) -> List[str]:
        ...
        atom_infos = self.retrieve_atom_info_through_atom(queries=query)
